...
import AlumniDirectory from "../common-components-management/AlumniDirectory";
...
export default function StudentDashboard() {
  ...
  const renderTab = () => {
        ...
        <div className="space-y-8">
          ...
          {data?.notifications && data.notifications.length > 0 && (
            <div className="space-y-4">
              {data.notifications.map((notif: any, idx: number) => (
                <div key={idx} className={`flex items-start gap-4 p-4 rounded-xl border ${notif.type === 'danger' ? 'bg-red-50 dark:bg-red-900/30 border-red-200 text-red-800 dark:text-red-400' : 'bg-amber-50 dark:bg-amber-900/30 border-amber-200 text-amber-800 dark:text-amber-400'}`}>
                  <AlertCircle className={`w-5 h-5 mt-0.5 shrink-0 ${notif.type === 'danger' ? 'text-red-600 dark:text-red-400' : 'text-amber-600 dark:text-amber-400'}`} />
                  <div>
              ...
              return (
                <div key={index} className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 hover:shadow-lg transition-shadow">
                  ...
                  <div className="mt-4 flex items-center gap-1 text-sm">
                    ...
                    <span className="text-gray-500 dark:text-gray-400 ml-1">status</span>
                  </div>
                </div>
              );
            })}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {[
                ...
                const colorClasses = {
                  blue: "bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/50",
                  amber: "bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 hover:bg-amber-100 dark:hover:bg-amber-900/50",
                  emerald: "bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 hover:bg-emerald-100 dark:hover:bg-emerald-900/50",
                }[action.color];

                return (
                  <button key={index} onClick={action.onClick} className={`flex items-start gap-4 p-4 rounded-lg border border-gray-200 dark:border-gray-700 transition-colors text-left ${colorClasses}`}>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setActiveTab("courses")} className="text-sm text-blue-600 hover:text-blue-700 dark:text-blue-400 font-medium">
                View all
              </button>
            </div>
            <div className="space-y-4">
              {data?.todayClasses && data.todayClasses.length > 0 ? (
                data.todayClasses.slice(0, 3).map((class_: any, index: number) => {
                  ...
                  return (
                    <div key={class_.id || index} className={`flex items-center gap-4 p-4 rounded-lg transition-colors ${isUpcoming ? "bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 shadow-sm" : "bg-gray-50 dark:bg-gray-900 border border-transparent"}`}>
                      <div className={`w-16 text-sm font-medium ${isUpcoming ? "text-blue-700 dark:text-blue-400" : "text-gray-700 dark:text-gray-400"}`}>{class_.time}</div>
                      <div className="flex-1">
                        <p className={`font-medium ${isUpcoming ? "text-blue-900 dark:text-blue-300" : "text-gray-900 dark:text-white"}`}>{class_.subject}</p>
                        <p className={`text-sm ${isUpcoming ? "text-blue-600 dark:text-blue-400" : "text-gray-500 dark:text-gray-400"}`}>{class_.faculty} • {class_.room} • {class_.type}</p>
                      </div>
                      {isUpcoming && <span className="px-2.5 py-1 text-xs font-semibold bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-300 rounded-full animate-pulse">Next</span>}
                    </div>
                  );
                })
              ) : (
    ...
    const containerClass = fullWidthTabs.includes(activeTab) ? "" : "bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6";
    ...
    return (
      <div className={containerClass}>
        ...
        {activeTab === "alumni-directory" && <AlumniDirectory />}
        ...
        {activeTab === "settings" && <div className="text-sm text-gray-600 dark:text-gray-400">Settings are not available yet for student accounts.</div>}
      </div>
    );
  };
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex items-center justify-center">
        <div className="text-center max-w-md mx-auto px-4">
          ...
          <p className="text-gray-600 dark:text-gray-400 mb-6">There was an error loading your dashboard. Please try again.</p>
          ...
          <div className="p-6 border-b border-gray-200 dark:border-gray-700">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{studentProgram}</p>
              </div>
            ...
            <div className="mt-6 p-4 bg-blue-50 dark:bg-blue-900/30 rounded-lg">
              <p className="font-medium text-gray-900 dark:text-white">{student?.name || "Student"}</p>
              <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">ID: {student?.studentId || "Not set"}</p>
            </div>
          </div>
          ...
          <div className="px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div className="relative hidden sm:block">
                ...
                <input placeholder="Search..." className="pl-9 pr-4 py-2 border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 w-64" />
              </div>
            </div>
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => setActiveTab("announcements")} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg relative">
